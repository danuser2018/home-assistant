...
- Sincronización de las skills transversales (`communication-patterns`, `event-driven-architecture`, `feature-refinement`, `service-responsibilities`, `system-deployment`, `architecture-decisions`) y de dominio (`audio-subsystem`) incorporando las referencias al nuevo `ADR-022` y los sujetos estandarizados NATS.
